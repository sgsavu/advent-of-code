const input = `tmmnhlxzpj1eightldxhjnone97
...
const part1 = () => {
    ...
    for (let i = 0; i < inputArr.length; i++) {
        ...
        for (let j = 0; j <= input.length; j++) {
            ...
            if (y === null) {
                const char = Number(input[input.length - j])
...
const part2 = () => {
    ...
    for (let i = 0; i < inputArr.length; i++) {
        ...
        const literalIndexToLiteralValue = {}

        Object.keys(literalToNumber).forEach(literal => {
            let literalIndex = input.indexOf(literal)
            let subInput = input

            while (literalIndex !== -1) {
                const indexAdjustment = input.length - subInput.length

                literalIndexToLiteralValue[literalIndex + indexAdjustment] = literalToNumber[literal]

                subInput = subInput.substring(literalIndex + literal.length)

                literalIndex = subInput.indexOf(literal)
            }
        })

        const literalArray = Object.entries(literalIndexToLiteralValue)

        const firstLiteral = literalArray?.[0]
        const lastLiteral = literalArray?.[literalArray.length - 1]

        const conditionFirst = firstLiteral ? (wow) => (wow < firstLiteral[0]) && !x : () => true 
        const conditionLast = lastLiteral ? (wow) => (wow < (input.length - lastLiteral[0])) && !y : () => true

        for (let j = 0; (j <= input.length) && (conditionFirst(j) || conditionLast(j)); j++) {


            if (x === null && conditionFirst(j)) {
                const char = Number(input[j])
        ...
        const firstDigit = firstLiteral && x === null ? firstLiteral[1] : x
        const lastDigit = lastLiteral && y === null ? lastLiteral[1] : y
